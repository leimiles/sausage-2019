...
Shader "Lux LWRP/Terrain/Blend"
{
    ...
    SubShader
    {
        ...
        //  Include base inputs and all other needed "base" includes
            #include "Includes/Lux LWRP Terrain Blend Inputs.hlsl"
        ...
        //  Vertex shader
            ...
            //  Pull positionCS.z towards camera
                ...
                #if UNITY_REVERSED_Z
                    vertexInput.positionCS.z += _Shift / max(_ProjectionParams.y, vertexInput.positionCS.w);
                #else
                    vertexInput.positionCS.z -= _Shift / max(_ProjectionParams.y, vertexInput.positionCS.w);
                #endif
            ...
            //  lets try world pos
                ...
                float3 tweakedWorldPos = vertexInput.positionWS;
                tweakedWorldPos += SafeNormalize(viewDirWS) * _Shift * 10 * saturate(length(viewDirWS) - _ProjectionParams.y * 10);
            // ok but goes crazy at lose distances??
//                vertexInput.positionCS = TransformWorldToHClip(tweakedWorldPos) ; //  * (length(viewDirWS));

                float3 forward = mul((float3x3)unity_CameraToWorld, float3(0,0,1));
                //tweakedWorldPos = vertexInput.positionWS - forward * _Shift * 0.5; // produces missing tris?

                //vertexInput.positionCS.zw = TransformWorldToHClip(tweakedWorldPos).zw;
